...
def setup():
    future_players_of_interest = []     # list of player IDs of future MTU players whose transfers we want to flag
    former_players_of_interest = []     # list of player IDs of former MTU players whose transfers we want to flag
    ...
    where_are_they_now_url = 'https://www.eliteprospects.com/team/548/michigan-tech/where-are-they-now?sort=tp'
    poi_data = requests.get(where_are_they_now_url)
    poi_html = BeautifulSoup(poi_data.text, 'html.parser')
    poi_player_tables = poi_html.select('div.expandable-table-wrapper')

    # Create a list of all future and former player IDs on Michigan Tech's "Where Are They Now?" page
    poi_player_pages_and_ids = re.findall(r'<a href=\"(https://www\.eliteprospects\.com/player/(\d*)/.*)\">.*</a>', str(poi_player_tables))
    
    # Separate the poi_player_links_and_ids list in to one for future players, and another for former ones
    for entry in poi_player_pages_and_ids:
        poi_page_data = requests.get(entry[0] + '?league=NCAA')
        ...
        if len(ncaa_section) == 1 and ncaa_section[0].find('td', {'class': 'regular gp'}).text == '-*':
            ...
            future_players_of_interest.append(entry[1])
        else:
            # The player is a former MTU player
            former_players_of_interest.append(entry[1])

    return future_players_of_interest, former_players_of_interest, transaction_ids_list

def process_feed(feed, future_players_of_interest, former_players_of_interest, transaction_ids_list):
    ...
    for item in feed.entries:
        ...
        if re.search(r'From: <a href="https:\/\/www.eliteprospects.com\/team\/548\/', decoded_description):
            ...
            for player_id in future_players_of_interest:
                player_id_regex = '/player/' + player_id
                if re.search(player_id_regex, decoded_description):
                    # A future Michigan Tech player is involved in this transfer in some way
                    process_match(transaction_id, transaction_ids_list, item.title, decoded_description, 'Future Player')

            for player_id in former_players_of_interest:
                player_id_regex = '/player/' + player_id
                if re.search(player_id_regex, decoded_description):
                    # A former Michigan Tech player is involved in this transfer in some way
                    process_match(transaction_id, transaction_ids_list, item.title, decoded_description, 'Former Player')

def main():
    feed = feedparser.parse('https://www.eliteprospects.com/rss/transfers')
    future_players_of_interest, former_players_of_interest, transaction_ids_list = setup()
    process_feed(feed, future_players_of_interest, former_players_of_interest, transaction_ids_list)
